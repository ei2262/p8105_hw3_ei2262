...
few days that the participant had counts over 5000. Majority of these
days were on a weekday rather than on a weekend. There is a dip in the
middle of the graph that represent activity counts less than 2500. It
may be worth exploring the isolated drop in activity counts.
...
numeric variables. The variables `prcp`, `snow`, and `snwd` were
converted to cm. `prcp` represents precipitation, `snow` represents
snowfall, and `snwd` represents snow depth, all in cm. `tmax` represents
maximum temperature and `tmin` represents minimum temperature, both in
degrees Celsius.
...
From the two-panel plot, we observe that July had higher average
...
colder than what would be expected in July.
